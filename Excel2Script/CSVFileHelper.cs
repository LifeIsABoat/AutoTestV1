...
namespace ScriptExcel2ScriptCSV
{
    public class CSVFileHelper
	{
        ...
        public static void DataTable2CSV(DataTable dt, string fullPath, Encoding encoding, bool utf8_WithBom)
		{
			...
			{
                data += AddDoubleQuotes(dt.Columns[i].ColumnName.ToString());
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				for (int j = 0; j < dt.Columns.Count; j++)
				{
                    ...
                    str = str.Replace("\"", "\"\""); // 对双引号转义,以便生成csv格式不会错乱
                    data += AddDoubleQuotes(str);
        ...
        /// <param name="CsvFileStorePath">CSV文件的存放路径</param>
        /// <param name="CsvFileName">CSV文件的名称</param>
        /// <param name="encoding">CSV的编码</param>
        /// <param name="utf8_WithBom">是否有Bom头</param>
        /// <param name="CutFileMaxCount">是否有Bom头</param>
        public static void Excel2CSV(string ExcelFilePath, string CsvFileStorePath, string CsvFileName, int CutFileMaxCount, Encoding encoding, bool utf8_WithBom)
        {
            DataTable data = Excel2DataTable(ExcelFilePath);
            int ScriptFileIndex = 0;
            string srtSavePath_CSV = "";
            DataTable tempdtInfo = new DataTable();

            //写出列名称
            for (int i = 0; i < data.Columns.Count; i++)
            {
                tempdtInfo.Columns.Add(data.Columns[i].ColumnName.ToString(), typeof(String));
            }

            if (0 < data.Rows.Count)
            {
                ScriptFileIndex = 0;
                srtSavePath_CSV = CsvFileStorePath + "\\";
                ...
                foreach (DataRow tempRow in data.Rows)
                {
                    tempdtInfo.ImportRow(tempRow);
                    if (tempdtInfo.Rows.Count >= CutFileMaxCount)
                    {
                        ScriptFileIndex++;
                        if((1 == ScriptFileIndex) && (tempdtInfo.Rows.Count >= data.Rows.Count))
                        {
                            DataTable2CSV(tempdtInfo, srtSavePath_CSV + CsvFileName + ".csv", encoding, utf8_WithBom);
                        }
                        else
                        {
                            DataTable2CSV(tempdtInfo, srtSavePath_CSV + CsvFileName + "_Part_" + ScriptFileIndex.ToString() + ".csv", encoding, utf8_WithBom);
                        }
                        
                        tempdtInfo.Rows.Clear();
                    }
                }
                if (tempdtInfo.Rows.Count > 0)
                {
                    ScriptFileIndex++;
                    if (1 == ScriptFileIndex)
                    {
                        DataTable2CSV(tempdtInfo, srtSavePath_CSV + CsvFileName + ".csv", encoding, utf8_WithBom);
                    }
                    else
        ...
        public static DataTable Excel2DataTable(string ExcelFilePath)
        {
            ...
            int intConditionIndex = 5;
            int intStepIndex = 6;
            int intExpectIndex = 7;
            int intSetpNumIndex = 16;
                    ...
                    if (isFirstRowColumn)
                    {
                        for (int i = firstRow.FirstCellNum; i < cellCount; ++i)
                        {
                            ...
                            if (cell != null)
                            {
                                ...
                                if (cellValue != null)
                                {
                                    if (true == cellValue.Equals("前置条件"))
                                    {
                                        intConditionIndex = i;
                                    }
                                    else if (true == cellValue.Equals("步骤"))
                                    {
                                        intStepIndex = i;
                                    }
                                    else if (true == cellValue.Equals("预期"))
                                    {
                                        intExpectIndex = i;
                                    }
                                    else if (true == cellValue.Equals("S"))
                                    {
                                        intSetpNumIndex = i;
                                    }
                                    else
                        ...
                        for (int j = row.FirstCellNum; j < cellCount; ++j)
                        {
                            ...
                            if (cell != null)
                            {
                                ...
                                else
                                {
                                    strTemp = "";
                                    /* 前置条件列 */
                                    if (intConditionIndex == j)
                                    {
                                        ArrCondition = row.GetCell(j).ToString().Split('\n');
                                        foreach (string strTempCondition in ArrCondition)
                                        {
                                            if (true == string.IsNullOrEmpty(strTempCondition.Trim()))
                                            {
                                                /* 前置条件中的空行，略过 */
                                                continue;
                                            }
                                            strTemp = strTemp + strTempCondition.Trim() + "\n";
                                        }
                                        if (1 < strTemp.Length)
                                        {
                                    ...
                                    /* 步骤列 */
                                    else if (intStepIndex == j)
                                    {
                                        ArrSteps = row.GetCell(j).ToString().Split('\n');
                                        stepCount = 0;
                                        foreach (string strTempStep in ArrSteps)
                                        {
                                            if (true == string.IsNullOrEmpty(strTempStep.Trim()))
                                            {
                                                /* 步骤中的空行，略过 */
                                                continue;
                                            }
                                            stepCount++;
                                            int iloop = 0;
                                            for (iloop = 0; iloop < strTempStep.Length; iloop++)
                                            {
                                                if (false == (('0' <= strTempStep[iloop]) && ('9' >= strTempStep[iloop]) || (' ' == strTempStep[iloop]) || ('.' == strTempStep[iloop])))
                                                {
                                                    break;
                                                }
                                            }
                                            strTemp = strTemp + stepCount.ToString() + ". " + strTempStep.Substring(iloop, strTempStep.Length - iloop).Trim() + "\n";
                                        }
                                        if (1 < strTemp.Length)
                                        {
                                            strTemp = strTemp.Remove(strTemp.Length - 1);
                                    ...
                                    /* 预期列 */
                                    else if (intExpectIndex == j)
                                    {
                                        if (0 < stepCount)
                                        {
                                            for(int iloop = 0; iloop < stepCount;iloop++)
                                            {
                                                strTemp = strTemp + (iloop + 1).ToString() + "." + "\n";
                                            }
                                            if (1 < strTemp.Length)
                                            {
                                                strTemp = strTemp.Remove(strTemp.Length - 1);
                                    ...
                                    /* S列 */
                                    else if (intSetpNumIndex == j)
                                    {
                                        dataRow[j] = stepCount.ToString();
                                    }
                                    else
                                    {
                                        dataRow[j] = row.GetCell(j).ToString();
        ...
        /* 字符串类型加双引号 */
        private static string AddDoubleQuotes(string strTemp)
        {
            return "\"" + strTemp + "\"";
        }
	}
